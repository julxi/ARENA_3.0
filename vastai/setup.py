...
# get env
load_dotenv(".env")
load_dotenv(".env.publish")
load_dotenv("vastai/.env")
...
with Connection(
    ...
    conn.run(
        f"uv pip install --python {remote_python} -r {remote_workdir}/{repo_name}/my_requirements.txt",
        warn=True,
    )
    conn.put(
        "chapter1_transformer_interp/exercises/.env",
        remote=f"{remote_workdir}/{repo_name}/chapter1_transformer_interp/exercises/.env",
    )
